...
# Apply the sigmoid function to 'Imon-1[A]' and add it as a new column
df_filtered.loc[:, 'Sigmoid(Imon-1[A])'] = df_filtered['Imon-1[A]'].apply(lambda x: sigmoid(x) if x > 0 else x)

# Create a directory to save the plots
output_dir = 'plots_avec_sigmoid_min_max'
os.makedirs(output_dir, exist_ok=True)

# Group by the unique positions (Y, Z, X)
grouped = df_filtered.groupby(['Y-pos[m]', 'Z-pos[m]', 'X-pos[m]'])

# Iterate over each group and plot I vs V with min to max zoom level
for name, group in grouped:
    Y_pos, Z_pos, X_pos = name
    min_value = group['Sigmoid(Imon-1[A])'].min()
    max_value = group['Sigmoid(Imon-1[A])'].max()
    
    # Identify the maximum value of Sigmoid(Imon-1[A])
    half_max_value = max_value / 2
    
    # Find the Emon-1[V] value corresponding to half_max_value
    half_max_emon = group.iloc[(group['Sigmoid(Imon-1[A])'] - half_max_value).abs().argmin()]['Emon-1[V]']
    ...
    imon_minus_0_05 = group.iloc[(group['Emon-1[V]'] - emon_minus_0_05).abs().argmin()]['Sigmoid(Imon-1[A])']
    imon_plus_0_05 = group.iloc[(group['Emon-1[V]'] - emon_plus_0_05).abs().argmin()]['Sigmoid(Imon-1[A])']

    plt.figure(figsize=(10, 6))
    plt.plot(group['Emon-1[V]'], group['Sigmoid(Imon-1[A])'], marker='o', linestyle='-')
    ...
    plt.title(f'Sigmoid(Imon-1[A]) vs Emon-1[V] for Y={Y_pos:.2E}, Z={Z_pos:.2E}, X={X_pos:.2E}')
    plt.xlabel('Emon-1[V]')
    plt.ylabel('Sigmoid(Imon-1[A])')
    plt.grid(True)
    plt.ylim([min_value - 0.5 * (max_value - min_value), max_value + 0.5 * (max_value - min_value)])  # Set y-axis limits from min to max with a 10% margin
    plt.legend()
...
for name, group in grouped:
    ...
    # Plot the maximum value line
    plt.axhline(y=max_value, color='r', linestyle='--', label=f'Plateau at {max_value}')
    
    # Plot the half maximum value line
    plt.axhline(y=half_max_value, color='g', linestyle='--', label=f'Plateau/2 at {half_max_value}')
    
    # Plot the vertical lines
    plt.axvline(x=half_max_emon, color='b', linestyle='--', label=f'Half Plateau Emon at {half_max_emon}V')
    plt.axvline(x=emon_minus_0_05, color='m', linestyle='--', label=f'Emon - 0.05V at {emon_minus_0_05}V')
    plt.axvline(x=emon_plus_0_05, color='m', linestyle='--', label=f'Emon + 0.05V at {emon_plus_0_05}V')
